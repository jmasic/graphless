...
type Worker interface {
	...
	HaltWorker(workerPayload domain.WorkerPayload)
	...
	InvokeOrchestratorFunction(payload string, workerPayload domain.WorkerPayload)
...
func New(payload domain.WorkerPayload, memoryClient memory.Client, messageClient message.Client, functionClient functionapi.Client) (Computer, error) {
	worker := &worker{
		runId:            payload.RunId,
...
func (w *worker) SendMessageToAllEdges(recipients []domain.Edge, message interface{}) {
	...
	w.messageClient.PutMessageForAllEdges(recipients, message, w.superstep)
}
...
func (w *worker) SendMessages(recipients []domain.Edge, messages []interface{}) {
	...
	w.messageClient.PutMessages(recipients, messages, w.superstep)
}
...
func (w *worker) SendMessage(recipient int64, message interface{}) {
	...
	w.messageClient.PutMessage(recipient, message, w.superstep)
}
...
func (w *worker) HaltWorker(workerPayload domain.WorkerPayload) {
	...
	if activeWorkersCount == 0 {
		w.InvokeOrchestratorFunction("SUPERSTEP FINISHED", workerPayload)
	}
}
...
func (w *worker) InvokeOrchestratorFunction(payload string, workerPayload domain.WorkerPayload) {
	log.Println("Invoking orchestrator function")
	binaryPayload, err := json.Marshal(domain.OrchestratorPayload{
		payload,
		workerPayload.MemoryClientConfig,
		workerPayload.MessageClientConfig,
		workerPayload.StorageClientConfig,
	})
