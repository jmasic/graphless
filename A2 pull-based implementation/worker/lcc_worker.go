...
func (msg LCCMessage) MarshalBinary() ([]byte, error) {
...
func (worker LCCWorker) sendConnectionInquiries(sourceVertexId int64, edgeList []int64) {
	...
	worker.SendMessageToAllEdges(edges, message)
}
...
func (worker LCCWorker) sendConnectionReplies(edges []domain.Edge, messages []LCCMessage) {
	...
	for index, message := range messages {
		...
		newMessages[index] = LCCMessage{SourceVertexId: message.SourceVertexId, MatchCount: matchCount}
	}
