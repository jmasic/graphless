...
then
        echo "Usage: redis-script.sh <number-of-instances> <user-id-for-redis-cli-access>"
        exit 1
fi
...
echo "PIDFile=/var/run/redis/redis_6379.pid" >> $REDIS.service
echo "ExecStart=\"/usr/local/bin/redis-server\" \"/etc/redis/redis_6379.conf\"" >> $REDIS.service
...
do
	 ...
	 if [ ! -f /etc/systemd/system/redis_$i.service ];then
	 	...
	 	sed -i "s/6379/$i/g" /etc/systemd/system/redis_$i.service
	 fi
done
