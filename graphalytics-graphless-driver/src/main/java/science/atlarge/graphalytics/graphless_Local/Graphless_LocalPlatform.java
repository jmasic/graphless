package science.atlarge.graphalytics.graphless_Local;
...
/**
 * Implementation for running the Graphalytics benchmark against a local instance of Graphless.
 *
 * @author jmasic
 */
public class Graphless_LocalPlatform implements Platform {
	...
	public LoadedGraph loadGraph(FormattedGraph formattedGraph) {
		// FIXME: Handle special case when already loaded (do nothing) to go faster
		final Path graphlessDirectory = resolveGraphlessDirectory();
		List<Path> vertexAndEdgePaths = copyToGraphlessFolder(formattedGraph, graphlessDirectory);
		compressGraphFiles(graphlessDirectory, vertexAndEdgePaths.get(0), formattedGraph.isDirected());

		return new LoadedGraph(
				formattedGraph,
				vertexAndEdgePaths.get(0).toAbsolutePath().toString(),
				vertexAndEdgePaths.get(1).toAbsolutePath().toString()
		);
	}
	...
	private List<Path> copyToGraphlessFolder(FormattedGraph formattedGraph, Path graphlessDirectory) {
		LOG.info("Graphless should be located at '{}'", graphlessDirectory);
		LOG.info("Formatted graph: '{}' '{}'", formattedGraph.getVertexFilePath(), formattedGraph.getEdgeFilePath());
		String[] graphNameTokens = formattedGraph.getName().split("\\.");
		String graphName = "graphalytics"; // graphNameTokens[0];
		LOG.info("Graph: '{}'", graphName);
		boolean isWeighted = graphNameTokens.length > 1 && graphNameTokens[1].contains("weight");
		Path graphsFolderPath = Paths.get(graphlessDirectory.toString(), "..", "simple-graphs", graphName);

		File graphsDirectory = graphsFolderPath.toFile();
		if (!graphsDirectory.exists()) {
			graphsDirectory.mkdir();
		}

		var sourceVertexFilePath = Paths.get(formattedGraph.getVertexFilePath());
		var sourceEdgeFilePath = Paths.get(formattedGraph.getEdgeFilePath());
		var destinationVertexFilePath = Paths.get(graphsDirectory.toString(), graphName + ".v");
		var destinationEdgeFilePath = Paths.get(graphsDirectory.toString(), graphName + ".e");
		try {
			Files.copy(sourceVertexFilePath, destinationVertexFilePath, StandardCopyOption.REPLACE_EXISTING);
			Files.copy(sourceEdgeFilePath, destinationEdgeFilePath, StandardCopyOption.REPLACE_EXISTING);
		} catch (IOException e) {
			throw new RuntimeException(e);
		}
		return List.of(destinationVertexFilePath, destinationEdgeFilePath);
	}

	private void compressGraphFiles(Path graphlessDirectory, Path vertexPath, boolean isDirected) {
		ProcessBuilder pb = new ProcessBuilder(
				"python3",
				"utils/to-json.py",
				vertexPath.toAbsolutePath().toString(),
				"50",
				isDirected ? "directed" : "undirected"
		);
		pb.directory(graphlessDirectory.toFile());
		LOG.info("Executing in directory '{}'...", graphlessDirectory);
		try {
	...
	public void prepare(RunSpecification runSpecification) {
		// TODO: Verify graphless folder *exists* and binaries exist
		// 		 Maybe something extra is needed? TBD
		File[] matchingFiles = getResultsDirectory().toFile().listFiles(
				(dir, name) -> name.contains("results")
		);
	...
	private Path getResultsDirectory() {
		return Paths.get(resolveGraphlessDirectory().toAbsolutePath().toString(), "..", "simple-graphs", "graphalytics", "s3");
	}
	...
	public void run(RunSpecification runSpecification) throws PlatformExecutionException {
		...
		GraphlessJobParams jobParams = new GraphlessJobParams(resolveGraphlessDirectory(), graph, GraphlessPlatform.LOCAL, runSpecification);
